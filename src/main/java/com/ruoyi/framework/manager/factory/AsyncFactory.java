...
public class AsyncFactory {
     ...
     * @return 任务task
     */
    public static TimerTask recordLogininfor(final String username, final String email, final String status, final String message, final Object... args) {
        final UserAgent userAgent = UserAgent.parseUserAgentString(ServletUtils.getRequest().getHeader("User-Agent"));
        final String ip = ShiroUtils.getIp();
        return new TimerTask() {
            @Override
            public void run() {
                ...
                if (Objects.equals(status, Constants.LOGIN_SUCCESS) && ToolUtils.isNotEmpty(email)) {
                    String text = "<p><strong class=\"ql-size-large\">登录通知</strong></p><p><strong>您的账号：fastBlog( " + username + " )已登陆</strong></p><p><br></p><p><strong>登录时间：" + LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")) + "</strong></p><p><strong>登录设备：" + os + "</strong></p><p><strong>登录IP：    " + ip + "</strong></p><p><strong>备注：如非本人操作，请立即访问网页版修改密码</strong></p>";
                    MailUtils.sendMailHtml(null, email, "登录成功", text);
                }
